MY_IP:=$(shell curl -sS ifconfig.me/all.json | jq .ip_addr)
export MY_IP
help:
...
cargo-test-ip: 	### 	cargo-test
	@. $(HOME)/.cargo/env
	#@cargo test
	./target/debug/slow_loris  $(MY_IP)
cargo-test-www-google-com:cargo-build 	### 	cargo-test
	@. $(HOME)/.cargo/env
	for ip in $(shell ip www.google.com); do echo $ip; ./target/debug/slow-loris $ip; done
